    ...
    with torch.set_grad_enabled(train):
        with tqdm.tqdm(total=len(dataloader)) as pbar:
            for (X, outcome) in dataloader:
                ...
                s1 += outcome.sum()
                s2 += (outcome ** 2).sum()
                print(f"s1 and s2 calculated, block_size: {block_size}")  # after this print the program crashes

                if block_size is None:
                    outputs = model(X)
                    print("model produced output")
                else:
                ...
                print("outputs updated according to average")
